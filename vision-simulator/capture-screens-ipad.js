...
const APP_URL = 'http://localhost:8936/index.html';
const OUT_DIR = '/Users/ryanevans/rjlcevans-ipad-screens/vision-simulator/screens';

// 13-inch iPad Pro (M4/M5) native pixels: 2064x2752 @ 2x scale factor.
...
(async () => {
  ...
  // ── Screen 1: Hero ──────────────────────────────────────────────
  {
    ...
    await shoot(page, 'ipad-s1-hero.png');
    await ctx.close();
  }
  ...
  // ── Screen 2: AMD compare ───────────────────────────────────────
  {
    ...
    await shoot(page, 'ipad-s2-amd-compare.png');
    await ctx.close();
  }
  ...
  // ── Screen 3: Cataract night ─────────────────────────────────────
  {
    ...
    await page.evaluate(() => window.scrollTo(0, 0));
    await shoot(page, 'ipad-s3-cataract-night.png');
    await ctx.close();
  }
  ...
  // ── Screen 4: Free/Pro conditions grid ────────────────────────────
  {
    ...
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.waitForTimeout(150);
    await shoot(page, 'ipad-s4-conditions-freepro.png');
    await ctx.close();
  }
  ...
  // ── Screen 5: Color blindness compare ─────────────────────────────
  {
    ...
    await enableCompare(page);
    await page.evaluate(() => window.scrollTo(0, 0));
    await shoot(page, 'ipad-s5-colorblind-compare.png');
    await ctx.close();
  }
